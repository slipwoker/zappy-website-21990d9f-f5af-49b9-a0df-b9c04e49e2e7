document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
      ...
      if (value !== '') {
        ...
        if (type === 'tel' || name.includes('phone') || name.includes('tel')) {
          ...
          if (!phonePattern.test(value)) {
            return field.dataset.errorPhone || 'Please enter a valid phone number.';
          }
        }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      // Optional: show success state (can be removed if server handles redirect)
  ...
  if (animatedEls.length > 0) {
    ...
    animatedEls.forEach(function (el) {
      ...
      const animClass = el.dataset.animate || el.className;
      const isRTL = document.documentElement.dir === 'rtl';

      if (animClass.includes('fade-in-up')) {
        el.style.transform = 'translateY(30px)';
      } else if (animClass.includes('fade-in-down')) {
        el.style.transform = 'translateY(-30px)';
      } else if (animClass.includes('fade-in-left')) {
        el.style.transform = isRTL ? 'translateX(30px)' : 'translateX(-30px)';
      } else if (animClass.includes('fade-in-right')) {
        el.style.transform = isRTL ? 'translateX(-30px)' : 'translateX(30px)';
      }
    });
    ...
    function revealOnScroll() {
      ...
      animatedEls.forEach(function (el) {
        ...
        if (rect.top < windowHeight - revealOffset) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "MB Finance By Maayan Bar Lev משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '21990d9f-f5af-49b9-a0df-b9c04e49e2e7',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '21990d9f-f5af-49b9-a0df-b9c04e49e2e7';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '21990d9f-f5af-49b9-a0df-b9c04e49e2e7';
